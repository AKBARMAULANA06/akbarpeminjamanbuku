...
<div class="container-fluid px-4 py-4" style="background: #faf7f2; min-height: 100vh;">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            ...
            <p class="mb-0" style="color: #8b7a6b;">
                <i class="bi bi-clock-history me-1"></i>Daftar semua peminjaman buku Anda
            </p>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-end mb-4">
        <a href="{{ route('user.peminjaman.create') }}" class="btn d-flex align-items-center px-4 py-2" 
           style="background: linear-gradient(135deg, #8b5a2b 0%, #a8753a 100%); color: white; border-radius: 50px; font-weight: 500; border: none;">
            <i class="bi bi-book-plus me-2"></i> Pinjam Buku Baru
        </a>
    </div>
                ...
                <table class="table align-middle mb-0" style="border-collapse: separate; border-spacing: 0;">
                    <thead style="background: #f8f5f0;">
                        <tr>
                            ...
                            <th class="py-3" style="color: #6b5c4d; font-weight: 600;">Jumlah</th>
                    ...
                    <tbody>
                        @forelse($peminjamans as $p)
                            <tr style="border-bottom: 1px solid #f0e7d8;">
                                ...
                                <td class="py-3">
                                    <span style="color: #6b5c4d;">{{ $p->tanggal_kembali_rencana->format('d/m/Y') }}</span>
                                </td>
                                <td class="py-3">
                                    <span class="badge py-2 px-3" style="background: #f0e7d8; color: #8b5a2b; border-radius: 30px;">
                                        <i class="bi bi-layers me-1"></i>{{ $p->jumlah }} eks.
                                    </span>
                                </td>
                                <td class="py-3">
                                    @php
                                    ...
                                    <span class="badge py-2 px-3" style="background: {{ $color['bg'] }}; color: {{ $color['text'] }}; border-radius: 30px;">
                                        <i class="bi 
                                            @if($p->status == 'pending') bi-hourglass-split
                                            @elseif($p->status == 'approved') bi-book
                                            @elseif($p->status == 'returned') bi-check-circle
                                            @else bi-x-circle
                                            @endif me-1">
                                        </i>
                                ...
                                <td class="py-3">
                                    @if($p->pengembalian)
                                        <span class="fw-bold {{ $p->pengembalian->denda > 0 ? 'text-danger' : 'text-success' }}">
                                            @if($p->pengembalian->denda > 0)
                                                <i class="bi bi-exclamation-triangle-fill me-1"></i>
                                            @else
                                                <i class="bi bi-check-circle-fill me-1"></i>
                                            @endif
                                            {{ $p->pengembalian->denda_formatted }}
                                        </span>
                                    @else
                                ...
                                <td class="text-end pe-4 py-3">
                                    <a href="{{ route('user.peminjaman.show', $p) }}" class="btn d-inline-flex align-items-center" 
                                       style="background: #f0e7d8; color: #8b5a2b; border-radius: 30px; padding: 8px 16px; font-weight: 500;">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-5">
                                    ...
                                    <p class="text-muted mb-3" style="color: #8b7a6b;">Belum ada peminjaman buku</p>
